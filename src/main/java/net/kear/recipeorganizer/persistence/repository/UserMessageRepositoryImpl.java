...
public class UserMessageRepositoryImpl implements UserMessageRepository {
	...
	public List<UserMessageDto> listMessages(long toUserId) {
    	SQLQuery query = (SQLQuery) getSession().createSQLQuery(
			"select m.id as id, m.to_user_id as toUserId, m.from_user_id as fromUserId, u.firstname as fromFirstName, u.lastname as fromLastName, m.message as message,"
					+ " m.viewed as viewed, m.recipe_id as recipeId, r.name as recipeName, m.date_sent as dateSent"
					+ " from message m left outer join users u on m.from_user_id = u.id left outer join recipe r on m.recipe_id = r.id"
