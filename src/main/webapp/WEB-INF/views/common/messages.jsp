<%@ taglib uri="http://www.springframework.org/tags" prefix="spring" %>

<div class="spring-messages" style="display:none">
	<p id="common.yes"/><spring:message code="common.yes"></spring:message>
	<p id="common.no"/><spring:message code="common.no"></spring:message>
	<p id="errordlg.title"/><spring:message code="errordlg.title"></spring:message>
	<p id="useradmin.delete.title"/><spring:message code="useradmin.delete.title"></spring:message>
	<p id="useradmin.delete.areyousure1"/><spring:message code="useradmin.delete.areyousure1"></spring:message>
	<p id="useradmin.delete.areyousure2"/><spring:message code="useradmin.delete.areyousure2"></spring:message>
	<p id="useradmin.delete.hasrecipe1"/><spring:message code="useradmin.delete.hasrecipe1"></spring:message>
	<p id="useradmin.delete.hasrecipe2"/><spring:message code="useradmin.delete.hasrecipe2"></spring:message>
	<p id="useradmin.table.emptyTable"/><spring:message code="useradmin.table.emptyTable"></spring:message>
	<p id="useradmin.table.info"/><spring:message code="useradmin.table.info"></spring:message>
	<p id="useradmin.table.infoEmpty"/><spring:message code="useradmin.table.infoEmpty"></spring:message>
	<p id="useradmin.table.infoFiltered"/><spring:message code="useradmin.table.infoFiltered"></spring:message>
	<p id="useradmin.table.lengthMenu"/><spring:message code="useradmin.table.lengthMenu"></spring:message>
	<p id="useradmin.table.zeroRecords"/><spring:message code="useradmin.table.zeroRecords"></spring:message>
	<p id="recipe.table.emptyTable"/><spring:message code="recipe.table.emptyTable"></spring:message>
	<p id="recipe.table.info"/><spring:message code="recipe.table.info"></spring:message>
	<p id="recipe.table.infoEmpty"/><spring:message code="recipe.table.infoEmpty"></spring:message>
	<p id="recipe.table.infoFiltered"/><spring:message code="recipe.table.infoFiltered"></spring:message>
	<p id="recipe.table.lengthMenu"/><spring:message code="recipe.table.lengthMenu"></spring:message>
	<p id="recipe.table.zeroRecords"/><spring:message code="recipe.table.zeroRecords"></spring:message>
	<p id="recipe.delete.title"/><spring:message code="recipe.delete.title"></spring:message>
	<p id="recipe.delete.areyousure"/><spring:message code="recipe.delete.areyousure"></spring:message>
	<p id="recipe.remove.favorite.title"/><spring:message code="recipe.remove.favorite.title"></spring:message>
	<p id="recipe.remove.favorite.areyousure1"/><spring:message code="recipe.remove.favorite.areyousure1"></spring:message>
	<p id="recipe.remove.favorite.areyousure2"/><spring:message code="recipe.remove.favorite.areyousure2"></spring:message>
	<p id="common.table.search"/><spring:message code="common.table.search"></spring:message>
	<p id="common.table.paginate.first"/><spring:message code="common.table.paginate.first"></spring:message>
	<p id="common.table.paginate.last"/><spring:message code="common.table.paginate.last"></spring:message>
	<p id="common.table.paginate.next"/><spring:message code="common.table.paginate.next"></spring:message>
	<p id="common.table.paginate.previous"/><spring:message code="common.table.paginate.previous"></spring:message>
</div>
